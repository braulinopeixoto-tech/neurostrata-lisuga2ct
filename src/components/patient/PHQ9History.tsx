...
export function PHQ9History({ patientId }: { patientId: string }) {
  ...
  return (
    <div className="space-y-4">
      {history.map((record) => (
        <Card key={record.id} className="shadow-sm border-l-4 border-l-purple-500">
          <CardContent className="p-4">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h4 className="font-semibold text-primary flex items-center gap-2">
                  <Activity className="w-4 h-4" /> Inventário PHQ-9
                </h4>
              ...
              <div className="text-right">
                ...
                <span className="text-xs text-muted-foreground font-mono ml-1">/ 27</span>
              </div>
            </div>
