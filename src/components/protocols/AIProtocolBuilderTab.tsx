...
import { Bot, Sparkles, Plus, Zap, ArrowRight, BrainCircuit } from 'lucide-react'
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in">
      <Card className="shadow-sm flex flex-col">
        <CardHeader>
          ...
          <CardDescription>
            Descreva o quadro do paciente e deixe a IA propor um sequenciamento terapêutico validado.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex-1 flex flex-col space-y-4">
          ...
          >
            {isGenerating ? (
              <span className="flex items-center gap-2 animate-pulse"><BrainCircuit className="w-4 h-4"/> Processando na NeuroStrata Engine...</span>
            ) : (
              <span className="flex items-center gap-2"><Sparkles className="w-4 h-4" /> Gerar Sequenciamento Estratégico</span>
            )}
          </Button>
        </CardContent>
      </Card>
      ...
      <div className="space-y-4">
        {!result ? (
          <Card className="h-full border-dashed bg-muted/20 flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
            ...
            <p className="max-w-xs">O sequenciamento sugerido pela IA aparecerá aqui após a análise estruturada.</p>
          </CardCard>
        ) : (
          <div className="space-y-4 animate-fade-in-up">
            ...
            {[result.phase1, result.phase2, result.phase3].map((phase, i) => (
              <Card key={i} className="shadow-sm border-l-4 border-l-accent overflow-hidden relative group">
                <CardContent className="p-5">
                  ...
                  <div className="space-y-2">
                    {phase.modules.map((mod: string, idx: number) => (
                      <div key={idx} className="flex items-center gap-2 text-sm bg-muted/30 p-2 rounded border">
                        <Plus className="w-3.5 h-3.5 text-accent" /> {mod}
                      </div>
                    ))}
                  </div>
                  
                  <Button variant="ghost" size="sm" className="mt-4 w-full opacity-0 group-hover:opacity-100 transition-opacity text-primary hover:bg-primary/10">
